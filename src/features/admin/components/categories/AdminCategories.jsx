...
const CategoriesSection = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err?.message || t("categories.categoriesSection.createError"));
    }
  };
  ...
  const remove = async (cat) => {
    ...
    try {
      ...
      setDeleteError({ id: cat.id, message: err?.status === 409 ? t("categories.categoriesSection.deleteErrorHasProducts") : t("categories.categoriesSection.deleteErrorGeneric") });
    }
  };
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: ".5px solid var(--line)", overflow: "hidden" }}>
      <div style={{ padding: "22px 26px" }}>
        <div className="display" style={{ fontSize: 19 }}>{t("categories.categoriesSection.title")}</div>
      </div>

      <form onSubmit={submit} style={{ display: "flex", gap: 10, padding: "0 26px 20px", flexWrap: "wrap" }}>
        <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder={t("categories.categoriesSection.namePlaceholder")} style={{ ...fieldStyle, flex: "1 1 160px" }} />
        <input value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))} placeholder={t("categories.categoriesSection.slugPlaceholder")} style={{ ...fieldStyle, flex: "1 1 140px" }} />
        <button type="submit" className="foc" style={{ border: 0, borderRadius: 999, padding: "10px 18px", background: "var(--ink)", color: "var(--cream)", fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>{t("categories.categoriesSection.add")}</button>
      </form>
      {error && <div style={{ fontSize: 12.5, color: "var(--terracotta-deep)", padding: "0 26px 14px" }}>{error}</div>}

      {status === "loading" && <div style={{ padding: "0 26px 20px", fontSize: 13, color: "var(--ink-soft)" }}>{t("categories.categoriesSection.loading")}</div>}
...
const AttributesSection = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err?.message || t("categories.attributesSection.createError"));
    }
  };
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: ".5px solid var(--line)", overflow: "hidden" }}>
      <div style={{ padding: "22px 26px" }}>
        <div className="display" style={{ fontSize: 19 }}>{t("categories.attributesSection.title")}</div>
      </div>

      <form onSubmit={submit} style={{ display: "flex", gap: 10, padding: "0 26px 20px", flexWrap: "wrap" }}>
        <input value={form.key} onChange={(e) => setForm((f) => ({ ...f, key: e.target.value }))} placeholder={t("categories.attributesSection.keyPlaceholder")} style={{ ...fieldStyle, flex: "1 1 140px" }} />
        <input value={form.label} onChange={(e) => setForm((f) => ({ ...f, label: e.target.value }))} placeholder={t("categories.attributesSection.labelPlaceholder")} style={{ ...fieldStyle, flex: "1 1 140px" }} />
        <input value={form.values} onChange={(e) => setForm((f) => ({ ...f, values: e.target.value }))} placeholder={t("categories.attributesSection.valuesPlaceholder")} style={{ ...fieldStyle, flex: "1 1 200px" }} />
        <button type="submit" className="foc" style={{ border: 0, borderRadius: 999, padding: "10px 18px", background: "var(--ink)", color: "var(--cream)", fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>{t("categories.attributesSection.add")}</button>
      </form>
      {error && <div style={{ fontSize: 12.5, color: "var(--terracotta-deep)", padding: "0 26px 14px" }}>{error}</div>}

      {status === "loading" && <div style={{ padding: "0 26px 20px", fontSize: 13, color: "var(--ink-soft)" }}>{t("categories.attributesSection.loading")}</div>}

...
const RULE_KEYS = { BEST_SELLERS: "ruleBestSellers", NEWEST: "ruleNewest" };
...
export const AdminFeaturedConfig = () => {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(err?.message || t("settings.featured.errorSave"));
    } finally {
      setSaving(false);
    }
  };

  if (status === "loading") return <div style={{ fontSize: 13, color: "var(--ink-soft)" }}>{t("settings.featured.loading")}</div>;
  if (status === "error") return <div style={{ fontSize: 13, color: "var(--terracotta-deep)" }}>{t("settings.featured.error")}</div>;

  return (
    <div style={{ background: "#fff", borderRadius: 20, border: ".5px solid var(--line)", padding: 24 }}>
      <div className="display" style={{ fontSize: 19, marginBottom: 18 }}>{t("settings.featured.title")}</div>

      <div style={{ display: "flex", gap: 20, marginBottom: 18 }}>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5, cursor: "pointer" }}>
          <input type="radio" checked={mode === "MANUAL"} onChange={() => setMode("MANUAL")} /> {t("settings.featured.manual")}
        </label>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5, cursor: "pointer" }}>
          <input type="radio" checked={mode === "AUTOMATIC"} onChange={() => setMode("AUTOMATIC")} /> {t("settings.featured.automatic")}
        </label>
      </div>
      ...
      {mode === "MANUAL" ? (
        ...
        <div style={{ marginBottom: 18 }}>
          <select value={automaticRule} onChange={(e) => setAutomaticRule(e.target.value)} style={{ padding: "10px 14px", border: "1px solid var(--line)", borderRadius: 10, background: "#fff", fontSize: 13.5 }}>
            {Object.entries(RULE_KEYS).map(([value, key]) => <option key={value} value={value}>{t(`settings.featured.${key}`)}</option>)}
          </select>
        </div>
      )}
      ...
      <button onClick={save} disabled={saving} className="foc" style={{ border: 0, borderRadius: 999, padding: "12px 22px", background: "var(--ink)", color: "var(--cream)", fontSize: 13, fontWeight: 700, cursor: saving ? "wait" : "pointer" }}>
        {saving ? t("settings.featured.saving") : saved ? t("settings.featured.saved") : t("settings.featured.save")}
      </button>
    </div>
  );
};

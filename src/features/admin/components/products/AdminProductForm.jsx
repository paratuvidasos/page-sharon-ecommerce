...
export const AdminProductForm = ({ product, onClose, onSaved }) => {
    ...
    try {
      ...
      if (savedId) {
        ...
        if (!newVariant.stockQuantity) {
          setError(t("products.form.errors.initialStockRequired"));
          setSaving(false);
          return;
        }
        payload.variants = [{
        ...
        if (pendingImages.length > 0) {
          ...
          try {
            ...
            setError(t("products.form.errors.imagesUploadFailedAfterCreate"));
          } finally {
            setUploading(false);
          }
        }
      ...
      setError(err?.message || t("products.form.errors.saveFailed"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpload = async (e) => {
    ...
    if (!savedId) {
      ...
      try {
        ...
        setError(err?.message || t("products.form.errors.previewGenerationFailed"));
      } finally {
    ...
    try {
      ...
      setError(err?.message || t("products.form.errors.uploadFailed"));
    } finally {
      setUploading(false);
      if (fileInputRef.current) fileInputRef.current.value = "";
    }
  ...
  const removeUploadedImage = async (url) => {
    ...
    try {
      ...
      setImages(prev);
      setError(err?.message || t("products.form.errors.removeImageFailed"));
    }
  };
  ...
  const addVariant = async () => {
    ...
    try {
      ...
      setError(t("products.form.errors.createVariantFailed"));
    }
  };

  const removeVariant = async (variantId) => {
    if (variants.length <= 1) {
      setError(t("products.form.errors.lastVariant"));
      return;
    }
    const prev = variants;
    ...
    try {
      ...
      setVariants(prev);
      setError(t("products.form.errors.removeVariantFailed"));
    }
  };

  return (
    <AdminFormSheet onClose={onClose} eyebrow={t("products.form.eyebrow")} title={savedId ? t("products.form.editTitle") : t("products.form.newTitle")}>
        <form onSubmit={submitDetails} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Field label={t("products.form.fields.name")}>
            <input value={details.name} onChange={(e) => setDetails((d) => ({ ...d, name: e.target.value }))} placeholder={t("products.form.fields.namePlaceholder")} style={fieldStyle} required />
          </Field>
          <Field label={t("products.form.fields.category")}>
            <select value={details.categoryId} onChange={(e) => setDetails((d) => ({ ...d, categoryId: e.target.value }))} style={fieldStyle} required>
              <option value="">{t("products.form.fields.categoryPlaceholder")}</option>
              {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
          </Field>
          <Field label={t("products.form.fields.description")}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label={t("products.form.fields.brand")}>
              <input value={details.brand} onChange={(e) => setDetails((d) => ({ ...d, brand: e.target.value }))} placeholder={t("products.form.fields.brandPlaceholder")} style={fieldStyle} />
            </Field>
            <Field label={t("products.form.fields.price")}>
              <div style={{ position: "relative" }}>
                ...
                <input value={details.basePrice} onChange={(e) => setDetails((d) => ({ ...d, basePrice: e.target.value }))} type="number" min="0" placeholder={t("products.form.fields.pricePlaceholder")} style={{ ...fieldStyle, paddingLeft: 26 }} required />
              </div>
            </Field>
          </div>

          {!savedId && (
            <Field label={t("products.form.fields.images")}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: pendingImages.length ? 10 : 0 }}>
                {pendingImages.map((p) => (
                  <div key={p.previewUrl} style={{ position: "relative", width: 56, height: 56, borderRadius: 10, overflow: "hidden", background: "var(--cream-2)" }}>
                    ...
                    <button type="button" onClick={() => removePendingImage(p.previewUrl)} aria-label={t("products.form.removeImageAria")} style={{ position: "absolute", top: 2, right: 2, width: 18, height: 18, borderRadius: "50%", background: "rgba(27,24,21,.75)", border: 0, color: "#fff", cursor: "pointer", display: "grid", placeItems: "center", padding: 0 }}>
          ...
          {!savedId && (
            <div style={{ border: "1px dashed var(--line)", borderRadius: 14, padding: 16 }}>
              <div className="eyebrow" style={{ fontSize: 10, marginBottom: 12 }}>{t("products.form.presentationSection")}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <Field label={t("products.form.fields.stock")}>
                  <input value={newVariant.stockQuantity} onChange={(e) => setNewVariant((v) => ({ ...v, stockQuantity: e.target.value }))} placeholder={t("products.form.fields.stockPlaceholder")} type="number" min="0" style={fieldStyle} />
                </Field>
                <Field label={t("products.form.fields.size")}>
                  <input value={newVariant.size} onChange={(e) => setNewVariant((v) => ({ ...v, size: e.target.value }))} placeholder={t("products.form.fields.sizePlaceholder")} style={fieldStyle} />
                </Field>
                <Field label={t("products.form.fields.scent")}>
                  <input value={newVariant.scent} onChange={(e) => setNewVariant((v) => ({ ...v, scent: e.target.value }))} placeholder={t("products.form.fields.scentPlaceholder")} style={fieldStyle} />
                </Field>
                <Field label={t("products.form.fields.color")}>
          ...
          <div className="admin-sheet-actions" style={{ display: "flex", gap: 10 }}>
            <button type="submit" disabled={saving} className="foc" style={{ flex: 1, border: 0, borderRadius: 999, padding: 14, background: "var(--ink)", color: "var(--cream)", fontSize: 13.5, fontWeight: 700, cursor: saving ? "wait" : "pointer" }}>
              {saving ? t("products.form.saving") : savedId ? t("products.form.saveChanges") : t("products.form.create")}
            </button>
            {!savedId && (
              <button type="button" onClick={onClose} className="foc" style={{ border: "1px solid var(--line)", borderRadius: 999, padding: "14px 20px", background: "transparent", color: "var(--ink)", fontSize: 13.5, fontWeight: 600, cursor: "pointer" }}>{t("products.form.cancel")}</button>
            )}
          </div>
        </form>
        ...
        {savedId && (
          <>
            ...
            <div className="eyebrow" style={{ fontSize: 10, marginBottom: 10 }}>{t("products.form.fields.images")}</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
              {images.map((url, i) => (
                <div key={i} style={{ position: "relative", width: 56, height: 56, borderRadius: 10, overflow: "hidden", background: "var(--cream-2)" }}>
                  ...
                  <button type="button" onClick={() => removeUploadedImage(url)} aria-label={t("products.form.removeImageAria")} style={{ position: "absolute", top: 2, right: 2, width: 18, height: 18, borderRadius: "50%", background: "rgba(27,24,21,.75)", border: 0, color: "#fff", cursor: "pointer", display: "grid", placeItems: "center", padding: 0 }}>
                    <Icon name="close" size={9} color="#fff" />
                  </button>
                </div>
              ))}
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="foc" style={{ display: "inline-flex", alignItems: "center", gap: 8, border: "1px solid var(--line)", borderRadius: 999, padding: "10px 16px", background: "#fff", fontSize: 12.5, fontWeight: 600, cursor: uploading ? "wait" : "pointer" }}>
              <Icon name="image" size={14} />
              {uploading ? t("products.form.uploading") : t("products.form.uploadImages")}
            </button>
            <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={handleUpload} style={{ display: "none" }} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}>
              <input value={newVariant.size} onChange={(e) => setNewVariant((v) => ({ ...v, size: e.target.value }))} placeholder={t("products.form.fields.size")} style={fieldStyle} />
              <input value={newVariant.stockQuantity} onChange={(e) => setNewVariant((v) => ({ ...v, stockQuantity: e.target.value }))} placeholder={t("products.form.fields.stock")} type="number" min="0" style={fieldStyle} />
            </div>
